...
if (length(args) > 3) {
  stop("Only three arguments should be provided: model(NB, logReg, RF, NN or xgboost), task_id and samplename")
}
...
if (deam.mut.ratio["deam"] > max.ratio*deam.mut.ratio["mut"]) { # if TRUE subsample deaminations
  ...
  X <- subs.XY[["X"]]
  Y <- subs.XY[["Y"]]
} else if (deam.mut.ratio["deam"] < max.ratio*deam.mut.ratio["mut"]) { # won't enter here never
