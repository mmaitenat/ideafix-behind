trainRF <- function(X, Y, seed.id) {
  ...
  model <- h2o.randomForest(
    ...
    model_id = "RESUBST",    ## name the model in H2O
  ...
  resubst.pred <- h2o.predict(object = model, 
                           newdata = train.h2o)
  resubst.out <- pROC::roc(Y, as.data.frame(resubst.pred)$X1, plot = FALSE)
  
  saveRDS(resubst.out, file = sprintf("RF_resubst-error_%s.RDS", seed.id))
}
