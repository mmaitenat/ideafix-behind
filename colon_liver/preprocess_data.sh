...
cat $run_accessions | while read sample; do
  ...
  trimmomatic PE -threads 6 $R1 $R2 $sample"_trimmed_R1.fastq" $sample"_unpaired_R1.fastq" $sample"_trimmed_R2.fastq" $sample"_unpaired_R2.fastq" ILLUMINACLIP:/opt/Trimmomatic-0.38/adapters/TruSeq3-PE-2.fa:2:30:10:10:TRUE SLIDINGWINDOW:4:20 LEADING:3 TRAILING:3 MINLEN:36
  mkdir -p aligned
  ...
  if cut -d"," -f1 $pair_filename | grep -q ${sample}; then
    ...
    $gatk4 Mutect2 \
       ...
       -I ${FF_bam_filename} \
       -normal ${FF_sample} \
       -O aligned/${sample}_vs_${FF_sample}.vcf \
       --annotation StrandBiasBySample

    # Mark TP and FPs with FilterMutectCalls. Calls that are likely true positives get the PASS label in the FILTER field, and calls that are likely false positives are labeled with the reason(s) for filtering in the FILTER field of the VCF
    echo "Filtering deaminations on sample ${sample}"
    $gatk4 FilterMutectCalls \
    -V aligned/${sample}_vs_${FF_sample}.vcf \
    -O aligned/${sample}_vs_${FF_sample}_filtermarks.vcf

    # Annotate SNPs
    echo "Annotating deaminations on sample ${sample}"
    java -jar $snpsift annotate $dbsnp aligned/${sample}_vs_${FF_sample}_filtermarks.vcf > aligned/${sample}_vs_${FF_sample}_filtermarks_annotated.vcf

    # Annotate homopolymers
    ## VCFPolyX
    $vcfpolyx_cmd -R ${REF} \
    --skip-filtered \
    -o aligned/${sample}_vs_${FF_sample}_VCFPolyx.vcf aligned/${sample}_vs_${FF_sample}_filtermarks_annotated.vcf

    ## VariantAnnotator - HomopolymerRun
    # The homopolymer length can only me computed for biallelic sites. Hence, the output vcf will be shorter than the input one
    $gatk3_cmd -T VariantAnnotator \
    -R ${REF} \
    -V aligned/${sample}_vs_${FF_sample}_filtermarks_annotated.vcf \
    -o aligned/${sample}_vs_${FF_sample}_HRun.vcf \
    -A HomopolymerRun \
    --reference_window_stop 200
fi
